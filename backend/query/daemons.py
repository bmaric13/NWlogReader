...
from backend.query.process_glossary import get_process_info, PROCESS_GLOSSARY
...
KEYWORD_RULES: list[tuple[re.Pattern, list[str], int]] = [
    ...
    (re.compile(r"\blink\s+(up|down|flap|not connected)\b", re.IGNORECASE), ["ethpm"],         2),
    (re.compile(r"\bSFP|transceiver|optic|DOM\b", re.IGNORECASE),         ["ethpm"],           2),
    (re.compile(r"\bCRC\b|\binput error|\bout error|overrun\b", re.IGNORECASE), ["ethpm", "hardware"], 2),
    ...
    (re.compile(r"\bvpc consistency|type.?1 inconsistency\b", re.IGNORECASE), ["vpc"],         2),

    # L2 forwarding
    (re.compile(r"\bMAC\s+(address|table|learn|aging)\b", re.IGNORECASE), ["l2fm"],           2),
    (re.compile(r"\bvlan\b.*\b(flood|forward|black.?hole)\b", re.IGNORECASE), ["l2fm"],       2),

    # BGP
    (re.compile(r"\bBGP\b",                   re.IGNORECASE),            ["bgp"],             1),
    (re.compile(r"\bneighbor.*reset|bgp.*down|bgp.*session\b", re.IGNORECASE), ["bgp"],       2),
    (re.compile(r"\bhold.?timer|notification\s+sent\b", re.IGNORECASE),   ["bgp"],             2),

    # OSPF
    (re.compile(r"\bOSPF\b",                  re.IGNORECASE),            ["ospf"],            1),
    (re.compile(r"\badjacency.*down|dead interval|DR election\b", re.IGNORECASE), ["ospf"],   2),
    ...
    (re.compile(r"\bRP\s+(address|reachable|unreachable)\b", re.IGNORECASE), ["pim"],         2),
    (re.compile(r"\bmulticast.*join|mroute|mrib\b", re.IGNORECASE),      ["pim"],             2),

    # ARP/ND
    (re.compile(r"\bARP\b",                   re.IGNORECASE),            ["arp"],             1),
    (re.compile(r"\bgratuitous ARP|arp.*conflict|duplicate IP\b", re.IGNORECASE), ["arp"],    2),
    (re.compile(r"\bneighbor discovery|ND\s+(entry|resolve)\b", re.IGNORECASE), ["arp"],      2),
    ...
    # Sysmgr / crash
    (re.compile(r"\bcore dump|process restarted|hap.?reset|killed.*signal\b", re.IGNORECASE), ["sysmgr"], 2),
...
def infer_daemons(
    ...
    for name, data in score_map.items():
        ...
        hint = DaemonHint(
            ...
            useful_commands=_substitute_commands(info.useful_commands if info else {}, element),
        )
        hints.append(hint)
    ...
    for platform, cmds in commands.items():
        result[platform] = [
            c.replace("{iface}", element)
             ...
             .replace("{ip}", element)
             .replace("{po_num}", element.lstrip("PortchannelPort-chanel").strip())
            for c in cmds
        ]
    return result
